...
class Yikes:
    ...
    def _get_import_chain(self, *, until: Optional[str] = None):
    ...
    def load_module(self, name):
        ...
        raise HelpfulError(
            'You are attempting to import requests, or import a module '
            'that uses requests. Requests (or any module that uses '
            'requests) should not be used in this code. See {} for why '
            'requests is not suitable for this code.'.format(
                '[https://discordpy.readthedocs.io/en/latest/faq.html'
                '#what-does-blocking-mean]'
            ),
            'Don\'t use requests, use aiohttp instead. The api is very '
            'similar to requests when using session objects. '
            '[http://aiohttp.readthedocs.io/en/stable/] If a module '
            'you\'re trying to use depends on requests, see if you can '
            'find a similar module compatable with asyncio. If you '
            'can\'t find one, learn how to avoid blocking in '
            'coroutines. If you\'re new to programming, consider '
            'learning more about how asynchronous code and coroutines '
            'work. Blocking calls (notably HTTP requests) can take a '
            'long time, during which the bot is unable to do anything '
            'but wait for it. If you\'re sure you know what you\'re '
            'doing, simply add `allow_requests = True` above your '
            'import statement, that being `import requests` or whatever '
            'requests dependent module.',
            footnote=(
...
fh = TimedRotatingFileHandler(
    filename=filename, when='midnight', encoding='utf-8'
)
fh.namer = lambda default_name: os.path.join(
    os.path.dirname(default_name),
    datetime.now().strftime('%Y%m%d.logs')
)
fh.setFormatter(
    logging.Formatter(
        '[{asctime}] [{levelname}] [{filename}, L.{lineno}] {message}',
        datefmt='%H:%M:%S',
        style='{',
    )
)
